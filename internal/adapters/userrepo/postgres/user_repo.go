...
func NewUserRepo(querier Querier) *UserRepo {
	return &UserRepo{querier: querier}
}

func (r *UserRepo) CreateUser(ctx context.Context, user auth.User) error {
	result, err := r.querier.ExecContext(ctx,
...
func (r *UserRepo) FindById(ctx context.Context, id string) (auth.User, error) {
	row := r.querier.QueryRowContext(ctx, "SELECT id, name, email, provider_user_id, avatar_url FROM users where id = $1", id)
