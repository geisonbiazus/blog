...
func TestCache(t *testing.T) {
	t.Run("Do", func(t *testing.T) {
		...
		t.Run("It caches the value and returns it on consecutive calls", func(t *testing.T) {
			...
			cache.Do("key", resolve, caching.NeverExpire)
			result, err := cache.Do("key", resolve, caching.NeverExpire)
		...
		t.Run("It does not cache the value when an error is returned", func(t *testing.T) {
			...
			cache.Do("key", resolve, caching.NeverExpire)
			result, returnedErr := cache.Do("key", resolve, caching.NeverExpire)
		...
		t.Run("It caches different values independently based on the key", func(t *testing.T) {
			...
			cache.Do("key1", resolve1, caching.NeverExpire)
			result1, _ := cache.Do("key1", resolve1, caching.NeverExpire)

			cache.Do("key2", resolve2, caching.NeverExpire)
			result2, _ := cache.Do("key2", resolve2, caching.NeverExpire)
